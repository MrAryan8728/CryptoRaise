import React from "react";

const Header = () => {
    return (
        <div>
            <hr/>
            <ul className=' flex gap-10 justify-between items-baseline'>
            <ul className=' flex gap-9 justify-evenly m-5 font-bold text-lg'>
                <li><img className = ' bg-red-400 h-8 w-8  rounded-md' src = ""/></li>
                <li >About</li>
                <li>Organisations</li>
                <li>Funds</li>
            </ul>
            <ul className=' flex gap-9 justify-evenly m-5 font-bold text-lg'>
                <li>
                   <div className=' flex m-0 border-x-2 border-y-2 rounded-md p-2'>
                       <input className=' border-white' type="text" placeholder="Search.."/>
                       <img className = 'h-9 w-8 px-2 py-3 font-bold text-lg'src='data:image/png;base64,'/>
                   </div>
                </li>
                <li><button className=' border-x-2 border-y-2 border-green-700 rounded-md px-3 py-2 text-green-400'>Login</button></li>
            </ul>
            </ul>
            <hr/>
        </div>
    )
}

export default Header;
